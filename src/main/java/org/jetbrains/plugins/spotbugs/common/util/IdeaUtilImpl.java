...
import org.jetbrains.plugins.spotbugs.collectors.AbstractClassAdder;
import org.jetbrains.plugins.spotbugs.collectors.ClassCollector;
...
public final class IdeaUtilImpl {
	...
	private static PsiClass findJavaPsiClass(final Project project, @NotNull final String dottedFqClassName, @NotNull final GlobalSearchScope searchScope) {
		 ...
		 */
		final PsiClass[] classes = ReadAction.compute(() -> JavaPsiFacade.getInstance(project).findClasses(dottedFqClassName, searchScope));
		if (classes.length > 0) {
			if (classes.length > 1) {
